...
function stubInvokeForOpenAi(modelId = 'whisper-1', secret: string | null = 'sk-test') {
    vi.mocked(core.invoke).mockImplementation(async (cmd: string, args?: unknown) => {
        if (cmd === 'get_setting') {
            const key = (args as { key: string }).key;
            if (key === 'activeProviderId') return 'openai';
            if (key === 'activeModelId') return modelId;
            return null;
        }
...
describe('functional: full transcribe flow', () => {
    ...
    it('throws when no API key is configured for the active provider', async () => {
        stubInvokeForOpenAi('whisper-1', null);
        const blob = new Blob([fixture('hello-world.wav')], { type: 'audio/wav' });
