---
description: Reset macOS Microphone, Accessibility, Input Monitoring, and Apple Events permissions for Vox Era (com.vhtechnology.voxera). Use when permission prompts no longer appear or to retest the grant flow.
---

Reset the macOS TCC entries for Vox Era so that Microphone, Accessibility, Input Monitoring, and Apple Events prompts fire again on next launch.
...
tccutil reset AppleEvents com.vhtechnology.voxera
```
...
> Permissions reset for Microphone, Accessibility, Input Monitoring, and Apple Events. Relaunch Vox Era from `/Applications/Vox Era.app` so macOS re-prompts for Microphone, then later for Accessibility and Input Monitoring (the latter two only fire when the global shortcut is pressed for the first time, with Input Monitoring specifically triggered when the Fn key is selected as the shortcut). Apple Events is declared in `Info.plist` for future paste-flow features and will prompt the first time Vox Era scripts another app. If no prompts appear, the bundle is missing entitlements — run `/diagnose`.
